...
from utils.utils import generate_data_FF, save_as_csv, make_plot_SNR

av_received_SNR = np.linspace(30, 30, 1)
...
def custom_tanh(x):
    return np.sqrt(1/output_nodes**2)*tf.nn.tanh(x)
...
for iters in range(len(std_noise_vector)):
    ...
    #train the cDNN
    history = model.fit([X_train[:,j] for j in range(2*n_users)], Y_train, epochs=6, batch_size=64)
    
    #save the loss for each epoch
    cDNN_loss_vs_epochs = history.history['loss']
